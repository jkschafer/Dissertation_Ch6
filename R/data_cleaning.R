...
datfile <- "./Data/Grueter_et_al_2015_Rooker&Gavrilets2018_combined_data.csv"
mainData <- read.csv(datfile, stringsAsFactors = FALSE)

vtd_longname <- "Visual.trait.dimorphism.score.without..sexual.dichromatism"
names(mainData)[names(mainData) == 
                  vtd_longname] <- "VTDwoSD"
longname2 <- "Visual.trait.dimorphism.score.value.with.sexual.dichromatism"
names(mainData)[names(mainData) == longname2] <- "VTDwSD"
...
# round VTD scores so that they are integers -> 
...
# Creating binary variable for ovulation signals -> absent, 
# slight, absent_slight = 0; present = 1
mainData$Ovulation_Signs_bin <- ifelse(mainData$Ovulation_Signs == 
                                         "Present",
                                       "1", "0")
...
mainData$Ovulation_Signs <- ifelse(mainData$Ovulation_Signs == 
                                   "Absent", 
                                   "0", 
...
reduced_data <- mainData[complete.cases(mainData[,c("Habitat", 
                                                    "Social.organization",
                                                    "Group.size", 
                                                    "Ovulation_Signs",
                                                    "VTDwSD", 
...
not_in_tree_reduced <- reduced_data[!reduced_data$Species %in% 
                                      tenKtree$tip.label,]
